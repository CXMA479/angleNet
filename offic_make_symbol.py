...
def get_vgg_train(num_classes=config.class_num, num_anchors=config.type_num):
	"""
    ...
    :return: Symbol
	"""
	data = mx.symbol.Variable(name="img")
	...
	rpn_conv = mx.symbol.Convolution(
        data=relu5_3, kernel=(3, 3), pad=(1, 1), num_filter=512, name=config.net.rpn_conv_name)
	rpn_relu = mx.symbol.Activation(data=rpn_conv, act_type="relu", name="rpn_relu")
	rpn_cls_score = mx.symbol.Convolution(
        data=rpn_relu, kernel=(1, 1), pad=(0, 0), num_filter=2 * num_anchors, name="rpn_cls_score")
	rpn_bbox_pred = mx.symbol.Convolution(
        data=rpn_relu, kernel=(1, 1), pad=(0, 0), num_filter=5 * num_anchors, name="rpn_bbox_pred")

	group = mx.sym.Group([rpn_cls_score,rpn_bbox_pred])
	return group
